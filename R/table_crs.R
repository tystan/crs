...
table_crs <- function(marg, rslv, trans_method = "probit") {

  n_trans <- length(trans_method)
  table_sens <- get_crs_sens(marg, rslv, trans_method = trans_method)
  table_sens <- bind_cols(tibble(param = rep("sens", n_trans)), table_sens)
  table_spec <- get_crs_spec(marg, rslv, trans_method = trans_method)
